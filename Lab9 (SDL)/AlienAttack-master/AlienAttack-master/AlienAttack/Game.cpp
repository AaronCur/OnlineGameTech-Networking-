...
Game::Game() : m_pWindow(), m_pRenderer(), m_bRunning(false), m_pGameStateMachine(),
m_playerLives(3), m_scrollSpeed(0.8), m_bLevelComplete(false), m_bChangingState(false)
{
    // add some level files to an array
    m_levelFiles.push_back(Constants::GFX_PATH + Constants::LEVEL1_FILENAME);
    m_levelFiles.push_back(Constants::GFX_PATH + Constants::LEVEL2_FILENAME);
