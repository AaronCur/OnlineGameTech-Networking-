...
{
    fileName = Constants::GFX_PATH + fileName;
...
void TextureManager::drawFrame(std::string id, int x, int y, int width, int height, int currentRow, int currentFrame, SDL_Renderer *pRenderer, double angle, int alpha, SDL_RendererFlip flip)
{
    ...
    if (SDL_SetTextureAlphaMod(m_textureMap[id], alpha) != 0)
    {
        std::cout << "SDL_SetTextureAlphaMod error: " << SDL_GetError() << std::endl;
    }

    if (SDL_RenderCopyEx(pRenderer, m_textureMap[id], &srcRect, &destRect, angle, 0, flip) != 0)
    {
        std::cout << "SDL_RenderCopyEx error: " << SDL_GetError() << std::endl;
    }
